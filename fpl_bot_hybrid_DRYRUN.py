...
def run_bot(team_id, test_mode=False):
    # ============ DRY-RUN BUILD — TEMPORARY ============
    # Ignores the deadline check so you can test now. Submits nothing,
    # emails nothing, saves nothing. Swap in fpl_bot_hybrid.py when done.
    # ===================================================
    if not test_mode:
        raise Exception("DRY-RUN build — paste fpl_bot_hybrid.py back in for live mode.")
    print("=" * 62)
    print("  DRY RUN — no submission, no email, nothing saved")
    print("=" * 62)
    ...
    if not should_update:
        ...
        if not unlimited:
            print("\nDeadline is far away and transfers are NOT unlimited —")
            print("the live bot would stop here. DRY RUN: carrying on anyway.\n")
        else:
    ...
    if unlimited and my_team is not None and len(my_team) >= 15:
        ...
        if test_mode:
            print("\nTEST MODE — nothing submitted to FPL, nothing saved.")
            print("Sending the email anyway so the poster can be checked.")
            send_advice_email(f"[DRY RUN] GW{gameweek} squad preview",
                              body, html, poster)
            return
    ...
    if test_mode:
        print("\nTEST MODE — nothing submitted to FPL, nothing saved.")
        print("Sending the email anyway so the poster can be checked.")
        send_advice_email(f"[DRY RUN] GW{gameweek} team preview", body, html, poster)
        return
...
def lambda_handler(event, context):
    ...
    try:
        # DRY-RUN BUILD: hard-coded True.
        run_bot(team_id, test_mode=True)
        print("\n" + "=" * 62)
        print("  DRY RUN FINISHED — nothing submitted, nothing sent")
        print("=" * 62)
