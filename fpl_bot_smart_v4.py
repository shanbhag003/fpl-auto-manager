...
def login(email, password):
    """Log in to FPL and return an authenticated session.

    Fails loudly and early if the login didn't take, so the bot doesn't quietly
    carry on with an anonymous session and then blow up somewhere confusing.
    """
    if not email or not password:
        raise Exception("FPL_EMAIL or FPL_PASSWORD env var is missing.")
    ...
    try:
        resp = session.post(url=login_url, data=login_data, timeout=REQUEST_TIMEOUT)
    except requests.RequestException as e:
        raise FPLUnavailable(f"Login request failed to even reach FPL: {type(e).__name__}: {e}")

    # FPL bounces you back to the login page with ?state=fail on bad credentials
    if 'state=fail' in resp.url:
        raise Exception("FPL rejected the login — wrong email or password.")

    if resp.status_code >= 400:
        raise Exception(
            f"Login returned HTTP {resp.status_code}. Body starts: {resp.text[:200]!r}"
        )

    if 'pl_profile' not in session.cookies.get_dict():
        print("[login] Warning: no pl_profile cookie set. Login may not have worked; "
              "will find out on the next call.")
...
def post_checked(session, url, label, **kwargs):
    ...
    The original code fired transfers and lineup changes without ever looking at
    the response, so a rejected transfer looked identical to a successful one.
    """
    kwargs.setdefault('timeout', REQUEST_TIMEOUT)
    try:
        resp = session.post(url=url, **kwargs)
    except requests.RequestException as e:
        raise FPLUnavailable(f"{label} request failed: {type(e).__name__}: {e}")

    if resp.status_code not in (200, 201, 204):
        raise Exception(
            f"{label} was REJECTED by FPL — HTTP {resp.status_code}. "
...
def update_team(email, password, team_id, test_mode=True):
    ...
    team_response = session.get(url, timeout=REQUEST_TIMEOUT)

    if team_response.status_code != 200:
        raise Exception(
            f"Could not fetch your team — HTTP {team_response.status_code} from /my-team/. "
            f"Body starts: {team_response.text[:200]!r}\n"
            "If this is 403, the login cookie wasn't accepted. Check FPL_EMAIL / FPL_PASSWORD, "
            "and make sure FPL_TEAM_ID is the entry ID that belongs to that login."
        )
    ...
    try:
        ...
        raise Exception(
            "The /my-team/ endpoint returned non-JSON — almost certainly a Cloudflare block page. "
            f"Body starts: {team_response.text[:200]!r}"
        )

    if 'picks' not in team:
        raise Exception(
            f"Logged in but got no 'picks' back. Response keys: {list(team.keys())}. "
            "Usually means FPL_TEAM_ID doesn't match the logged-in account."
        )
    ...
    if player_in is not None:
        ...
        if gain >= required_gain:
            ...
            if not test_mode:
                ...
                transfer_payload = {"transfers": transfers, "chip": None,
                                    "entry": int(team_id), "event": int(gameweek)}
                post_checked(session,
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # FPL's side is down or blocking us. Nothing was changed. Worth retrying
        # on the next scheduled run rather than treating it as a code bug.
        message = (
            "The FPL API was unreachable this run, so nothing was changed.\n\n"
            f"{e}\n\n"
            "This is usually temporary. The bot will try again on its next scheduled run."
        )
        print(message)
